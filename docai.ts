...
export default function Home() {
  // Form fields (match the existing UI exactly)
  ...
  function generateDemo() {
    ...
    // Build a sample row matching the user's runsheet format
  ...
  function exportToExcel() {
    ...
    // Build the worksheet matching BOP's exact runsheet format
    // Header rows mimic the existing template's "Chain of Title" sheet
    ...
    ws['!cols'] = [
      { wch: 18 }, // VOL/PAGE
      { wch: 22 }, // Instrument Type
      { wch: 22 }, // Doc/Recorded Date
      { wch: 35 }, // Grantor
      { wch: 35 }, // Grantee
      { wch: 50 }, // Description
      { wch: 40 }, // Comments
  ...
  return (
    <div className="container">
      ...
      {/* Form section — matches existing UI exactly */}
                  ...
                  <tr key={i} className={`conf-${row.confidence}`}>
                    <td style={{ fontSize: 11 }}>
                      ...
                      {row.notes_for_reviewer && (
                        <div style={{ color: '#991B1B', marginTop: 4, fontStyle: 'italic' }}>
                          ⚠ {row.notes_for_reviewer}
                        </div>
                      )}
                    </td>
                    <td>
                      <textarea value={row.vol_page} onChange={(e) => updateRow(i, 'vol_page', e.target.value)} />
                    </td>
                    <td>
                      <textarea value={row.instrument_type} onChange={(e) => updateRow(i, 'instrument_type', e.target.value)} />
                    </td>
                    <td>
                      <textarea value={row.doc_date} onChange={(e) => updateRow(i, 'doc_date', e.target.value)} />
                    </td>
                    <td>
                      <textarea value={row.recorded_date} onChange={(e) => updateRow(i, 'recorded_date', e.target.value)} />
                    </td>
                    <td>
                      <textarea value={row.grantor} onChange={(e) => updateRow(i, 'grantor', e.target.value)} />
                    </td>
                    <td>
                      <textarea value={row.grantee} onChange={(e) => updateRow(i, 'grantee', e.target.value)} />
                    </td>
                    <td>
                      <textarea value={row.description} onChange={(e) => updateRow(i, 'description', e.target.value)} />
                    </td>
                    <td>
                      <textarea value={row.comments} onChange={(e) => updateRow(i, 'comments', e.target.value)} />
                    </td>
                    <td className="row-actions">
                      <button className="btn-small" onClick={() => deleteRow(i)}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
